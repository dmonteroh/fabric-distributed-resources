...
func main() {
	...
	r := gin.Default()
	r.Use(contractMiddleware(contract))

	r.GET("/assets", func(c *gin.Context) {
		contract, ok := c.MustGet("fabric-contract").(*gateway.Contract)
		if !ok {
			log.Fatal("Fabric Contact unavailable in Gin Middleware")
		}
...
func contractMiddleware(contract *gateway.Contract) gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Set("fabric-contract", contract)
		c.Next()
	}
}
